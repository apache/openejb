  ...
  <xsl:template match="/document">
    ...
    <xsl:call-template name="make.page">
      <xsl:with-param name="css">
        <xsl:choose>
          <xsl:when test="/document/properties/style">
            <xsl:value-of select="/document/properties/style"/>
          </xsl:when>
          <xsl:otherwise>default.css</xsl:otherwise>
        </xsl:choose>
      </xsl:with-param>
      ...
      <xsl:with-param name="main-content">
        ...
        <xsl:call-template name="create.main"/>
      </xsl:with-param>
    ...
    <table cols="1" rows="2" border="0" cellpadding="0" cellspacing="0" width="430">
      <tr>
        <td valign="top" align="left"><br/>
           ...
           <xsl:if test="$title">
             <span class="pageTitle"><xsl:value-of select="$title"/></span><br/>
           </xsl:if>
        </td>
      </tr>
    </table>
