...
public final class WrapperClassGenerator extends ASMHelper {
    ...
    private void generateXmlJavaTypeAdapter(AnnotationVisitor av, XmlJavaTypeAdapter adapter) {
        if (adapter.value() != null) {
            av.visit("value", org.apache.xbean.asm.Type.getType(getClassCode(adapter.value())));
        }
        if (adapter.type() != javax.xml.bind.annotation.adapters.XmlJavaTypeAdapter.DEFAULT.class) {
            av.visit("type", org.apache.xbean.asm.Type.getType(getClassCode(adapter.type())));
        }
    }
    ...
    private void generateMessagePart(ClassWriter cw, MessagePartInfo mpi, Method method, String className) {
        ...
        mv.visitInsn(org.apache.xbean.asm.Type.getType(classCode).getOpcode(Opcodes.IRETURN));
        mv.visitMaxs(0, 0);
        mv.visitEnd();
        ...
        mv.visitCode();
        mv.visitVarInsn(Opcodes.ALOAD, 0);
        org.apache.xbean.asm.Type setType = org.apache.xbean.asm.Type.getType(classCode);
